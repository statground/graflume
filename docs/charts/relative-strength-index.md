...
![Current Relative strength index output](../assets/charts/relative-strength-index.svg)

This page documents the currently implemented **Relative strength index** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **relative strength index** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
relativeStrengthIndex('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · relative-strength-index',
  },
  accessibility: {
    label: 'Relative strength index example',
    description:
      'A compiled relative strength index example using the relative-strength-index family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Relative strength index example",
    "description": "A compiled relative strength index example using the relative-strength-index family."
  }
}
```
...
- User-facing family: `relative-strength-index`
- Quick API: `relativeStrengthIndex()`
- Portable mark: `indicator`
- Canonical family: `relative-strength-index`
- Category: `indicator`
