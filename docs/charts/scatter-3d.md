...
![Current Three-axis scatter chart output](../assets/charts/scatter-3d.svg)

This page documents the currently implemented **Three-axis scatter chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **three-axis scatter chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
scatter3d('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'cartesian · scatter-3d',
  },
  accessibility: {
    label: 'Three-axis scatter chart example',
    description: 'A compiled three-axis scatter chart example using the scatter-3d family.',
  },
});
```
...
- User-facing family: `scatter-3d`
- Quick API: `scatter3d()`
- Portable mark: `scatter-3d`
- Canonical family: `scatter-3d`
- Category: `cartesian`
...
Projects the third quantitative channel into position, size, depth order, and shading. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
...
Depth is compiled into portable 2D Scene geometry. A camera, occlusion engine, lighting model, and GPU picking are not part of ChartSpec 0.1.
