...
![Current Percentage price oscillator output](../assets/charts/percentage-price-oscillator.svg)

This page documents the currently implemented **Percentage price oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **percentage price oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
percentagePriceOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · percentage-price-oscillator',
  },
  accessibility: {
    label: 'Percentage price oscillator example',
    description:
      'A compiled percentage price oscillator example using the percentage-price-oscillator family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Percentage price oscillator example",
    "description": "A compiled percentage price oscillator example using the percentage-price-oscillator family."
  }
}
```
...
- User-facing family: `percentage-price-oscillator`
- Quick API: `percentagePriceOscillator()`
- Portable mark: `indicator`
- Canonical family: `percentage-price-oscillator`
- Category: `indicator`
