...
![Current Disparity index output](../assets/charts/disparity-index.svg)

This page documents the currently implemented **Disparity index** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **disparity index** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
disparityIndex('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · disparity-index',
  },
  accessibility: {
    label: 'Disparity index example',
    description: 'A compiled disparity index example using the disparity-index family.',
  },
});
```
...
- User-facing family: `disparity-index`
- Quick API: `disparityIndex()`
- Portable mark: `indicator`
- Canonical family: `disparity-index`
- Category: `indicator`
