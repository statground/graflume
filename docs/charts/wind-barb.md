...
![Current Wind barb chart output](../assets/charts/wind-barb.svg)

This page documents the currently implemented **Wind barb chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **wind barb chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
windBarb('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'cartesian · wind-barb',
  },
  accessibility: {
    label: 'Wind barb chart example',
    description: 'A compiled wind barb chart example using the wind-barb family.',
  },
});
```
...
- User-facing family: `wind-barb`
- Quick API: `windBarb()`
- Portable mark: `wind-barb`
- Canonical family: `wind-barb`
- Category: `cartesian`
...
Converts speed and direction into a shaft plus a bounded number of feather marks. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
