...
![Current Normalized average true range output](../assets/charts/normalized-average-true-range.svg)

This page documents the currently implemented **Normalized average true range** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **normalized average true range** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
normalizedAverageTrueRange('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · normalized-average-true-range',
  },
  accessibility: {
    label: 'Normalized average true range example',
    description:
      'A compiled normalized average true range example using the normalized-average-true-range family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Normalized average true range example",
    "description": "A compiled normalized average true range example using the normalized-average-true-range family."
  }
}
```
...
- User-facing family: `normalized-average-true-range`
- Quick API: `normalizedAverageTrueRange()`
- Portable mark: `indicator`
- Canonical family: `normalized-average-true-range`
- Category: `indicator`
