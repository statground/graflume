...
![Current Chande momentum oscillator output](../assets/charts/chande-momentum-oscillator.svg)

This page documents the currently implemented **Chande momentum oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **chande momentum oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
chandeMomentumOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · chande-momentum-oscillator',
  },
  accessibility: {
    label: 'Chande momentum oscillator example',
    description:
      'A compiled chande momentum oscillator example using the chande-momentum-oscillator family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Chande momentum oscillator example",
    "description": "A compiled chande momentum oscillator example using the chande-momentum-oscillator family."
  }
}
```
...
- User-facing family: `chande-momentum-oscillator`
- Quick API: `chandeMomentumOscillator()`
- Portable mark: `indicator`
- Canonical family: `chande-momentum-oscillator`
- Category: `indicator`
