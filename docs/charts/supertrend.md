...
![Current Supertrend output](../assets/charts/supertrend.svg)

This page documents the currently implemented **Supertrend** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **supertrend** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
supertrend('#chart', data, {
  ...
  title: {
    text: 'Supertrend',
    subtitle: 'indicator · supertrend',
  },
  accessibility: {
    label: 'Supertrend example',
    description: 'A compiled supertrend example using the supertrend family.',
  },
});
```
...
- User-facing family: `supertrend`
- Quick API: `supertrend()`
- Portable mark: `indicator`
- Canonical family: `supertrend`
- Category: `indicator`
