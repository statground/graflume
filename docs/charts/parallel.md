...
Axis brushing, drag reordering, inversion, bundled coordinate polylines, density mode, and built-in
legends are not implemented yet. Categorical dimensions and proportional stacked ribbons are
implemented through `mode: 'categories'`; they are an aggregated view rather than a lossless
replacement for an unbounded high-cardinality table.
