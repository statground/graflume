...
![Current Rate of change output](../assets/charts/rate-of-change.svg)

This page documents the currently implemented **Rate of change** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **rate of change** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
rateOfChange('#chart', data, {
  ...
  title: {
    text: 'Rate of change',
    subtitle: 'indicator · rate-of-change',
  },
  accessibility: {
    label: 'Rate of change example',
    description: 'A compiled rate of change example using the rate-of-change family.',
  },
});
```
...
- User-facing family: `rate-of-change`
- Quick API: `rateOfChange()`
- Portable mark: `indicator`
- Canonical family: `rate-of-change`
- Category: `indicator`
