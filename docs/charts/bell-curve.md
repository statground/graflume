...
![Current Bell curve output](../assets/charts/bell-curve.svg)

This page documents the currently implemented **Bell curve** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this distribution chart when the visual relationship represented by **bell curve** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
bellCurve('#chart', data, {
  ...
  title: {
    text: 'Bell curve',
    subtitle: 'distribution · bell-curve',
  },
  accessibility: {
    label: 'Bell curve example',
    description: 'A compiled bell curve example using the bell-curve family.',
  },
});
```
...
- User-facing family: `bell-curve`
- Quick API: `bellCurve()`
- Portable mark: `distribution`
- Canonical family: `bell-curve`
...
Derives mean and sample deviation, then renders a sampled density curve and fill. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
