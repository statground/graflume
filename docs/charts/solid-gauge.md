...
![Current Solid gauge output](../assets/charts/solid-gauge.svg)

This page documents the currently implemented **Solid gauge** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this radial chart when the visual relationship represented by **solid gauge** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
solidGauge('#chart', data, {
  ...
  title: {
    text: 'Solid gauge',
    subtitle: 'radial · solid-gauge',
  },
  accessibility: {
    label: 'Solid gauge example',
    description: 'A compiled solid gauge example using the solid-gauge family.',
  },
});
```
...
- User-facing family: `solid-gauge`
- Quick API: `solidGauge()`
- Portable mark: `solid-gauge`
- Canonical family: `solid-gauge`
- Category: `radial`
...
Maps values into concentric annular sectors between configured minimum and maximum angles. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
