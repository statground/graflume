...
![Current Area range chart output](../assets/charts/area-range.svg)

This page documents the currently implemented **Area range chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **area range chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
areaRange('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'cartesian · area-range',
  },
  accessibility: {
    label: 'Area range chart example',
    description: 'A compiled area range chart example using the area-range family.',
  },
});
```
...
- User-facing family: `area-range`
- Quick API: `areaRange()`
- Portable mark: `range`
- Canonical family: `area-range`
- Category: `cartesian`
