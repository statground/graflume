...
![Current Linear regression intercept output](../assets/charts/linear-regression-intercept.svg)

This page documents the currently implemented **Linear regression intercept** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **linear regression intercept** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
linearRegressionIntercept('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · linear-regression-intercept',
  },
  accessibility: {
    label: 'Linear regression intercept example',
    description:
      'A compiled linear regression intercept example using the linear-regression-intercept family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Linear regression intercept example",
    "description": "A compiled linear regression intercept example using the linear-regression-intercept family."
  }
}
```
...
- User-facing family: `linear-regression-intercept`
- Quick API: `linearRegressionIntercept()`
- Portable mark: `indicator`
- Canonical family: `linear-regression-intercept`
- Category: `indicator`
