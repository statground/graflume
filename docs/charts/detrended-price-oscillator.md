...
![Current Detrended price oscillator output](../assets/charts/detrended-price-oscillator.svg)

This page documents the currently implemented **Detrended price oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **detrended price oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
detrendedPriceOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · detrended-price-oscillator',
  },
  accessibility: {
    label: 'Detrended price oscillator example',
    description:
      'A compiled detrended price oscillator example using the detrended-price-oscillator family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Detrended price oscillator example",
    "description": "A compiled detrended price oscillator example using the detrended-price-oscillator family."
  }
}
```
...
- User-facing family: `detrended-price-oscillator`
- Quick API: `detrendedPriceOscillator()`
- Portable mark: `indicator`
- Canonical family: `detrended-price-oscillator`
- Category: `indicator`
