...
![Current Volume weighted average price output](../assets/charts/volume-weighted-average-price.svg)

This page documents the currently implemented **Volume weighted average price** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **volume weighted average price** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
volumeWeightedAveragePrice('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · volume-weighted-average-price',
  },
  accessibility: {
    label: 'Volume weighted average price example',
    description:
      'A compiled volume weighted average price example using the volume-weighted-average-price family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Volume weighted average price example",
    "description": "A compiled volume weighted average price example using the volume-weighted-average-price family."
  }
}
```
...
- User-facing family: `volume-weighted-average-price`
- Quick API: `volumeWeightedAveragePrice()`
- Portable mark: `indicator`
- Canonical family: `volume-weighted-average-price`
- Category: `indicator`
