...
![Current Stochastic oscillator output](../assets/charts/stochastic.svg)

This page documents the currently implemented **Stochastic oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **stochastic oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
stochastic('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · stochastic',
  },
  accessibility: {
    label: 'Stochastic oscillator example',
    description: 'A compiled stochastic oscillator example using the stochastic family.',
  },
});
```
...
- User-facing family: `stochastic`
- Quick API: `stochastic()`
- Portable mark: `indicator`
- Canonical family: `stochastic`
- Category: `indicator`
