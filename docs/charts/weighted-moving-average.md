...
![Current Weighted moving average output](../assets/charts/weighted-moving-average.svg)

This page documents the currently implemented **Weighted moving average** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **weighted moving average** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
weightedMovingAverage('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · weighted-moving-average',
  },
  accessibility: {
    label: 'Weighted moving average example',
    description:
      'A compiled weighted moving average example using the weighted-moving-average family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Weighted moving average example",
    "description": "A compiled weighted moving average example using the weighted-moving-average family."
  }
}
```
...
- User-facing family: `weighted-moving-average`
- Quick API: `weightedMovingAverage()`
- Portable mark: `indicator`
- Canonical family: `weighted-moving-average`
- Category: `indicator`
