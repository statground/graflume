...
![Current Directional movement index output](../assets/charts/directional-movement-index.svg)

This page documents the currently implemented **Directional movement index** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **directional movement index** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
directionalMovementIndex('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · directional-movement-index',
  },
  accessibility: {
    label: 'Directional movement index example',
    description:
      'A compiled directional movement index example using the directional-movement-index family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Directional movement index example",
    "description": "A compiled directional movement index example using the directional-movement-index family."
  }
}
```
...
- User-facing family: `directional-movement-index`
- Quick API: `directionalMovementIndex()`
- Portable mark: `indicator`
- Canonical family: `directional-movement-index`
- Category: `indicator`
