...
Horizontal legends include their outer padding and an eight-pixel gap between item controls in responsive measurement. Label widths follow the authored legend font size and conservative grapheme metrics for Latin, CJK, emoji, and RTL scripts. Labels that fit the available surface remain complete; constrained labels receive an ellipsis at a grapheme boundary and stay clipped within their own item. The full label remains available to the accessibility description and interactive button. Wrapping uses the same item widths as painting and hit regions, preserving separate pointer and keyboard controls without host-specific spacing.
