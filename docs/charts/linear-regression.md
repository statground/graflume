...
![Current Linear regression output](../assets/charts/linear-regression.svg)

This page documents the currently implemented **Linear regression** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **linear regression** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
linearRegression('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · linear-regression',
  },
  accessibility: {
    label: 'Linear regression example',
    description: 'A compiled linear regression example using the linear-regression family.',
  },
});
```
...
- User-facing family: `linear-regression`
- Quick API: `linearRegression()`
- Portable mark: `indicator`
- Canonical family: `linear-regression`
- Category: `indicator`
