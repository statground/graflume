...
![Current Commodity channel index output](../assets/charts/commodity-channel-index.svg)

This page documents the currently implemented **Commodity channel index** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **commodity channel index** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
commodityChannelIndex('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · commodity-channel-index',
  },
  accessibility: {
    label: 'Commodity channel index example',
    description:
      'A compiled commodity channel index example using the commodity-channel-index family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Commodity channel index example",
    "description": "A compiled commodity channel index example using the commodity-channel-index family."
  }
}
```
...
- User-facing family: `commodity-channel-index`
- Quick API: `commodityChannelIndex()`
- Portable mark: `indicator`
- Canonical family: `commodity-channel-index`
- Category: `indicator`
