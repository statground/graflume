...
![Current Pivot points output](../assets/charts/pivot-points.svg)

This page documents the currently implemented **Pivot points** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **pivot points** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
pivotPoints('#chart', data, {
  ...
  title: {
    text: 'Pivot points',
    subtitle: 'indicator · pivot-points',
  },
  accessibility: {
    label: 'Pivot points example',
    description: 'A compiled pivot points example using the pivot-points family.',
  },
});
```
...
- User-facing family: `pivot-points`
- Quick API: `pivotPoints()`
- Portable mark: `indicator`
- Canonical family: `pivot-points`
- Category: `indicator`
