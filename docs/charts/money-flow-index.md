...
![Current Money flow index output](../assets/charts/money-flow-index.svg)

This page documents the currently implemented **Money flow index** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **money flow index** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
moneyFlowIndex('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · money-flow-index',
  },
  accessibility: {
    label: 'Money flow index example',
    description: 'A compiled money flow index example using the money-flow-index family.',
  },
});
```
...
- User-facing family: `money-flow-index`
- Quick API: `moneyFlowIndex()`
- Portable mark: `indicator`
- Canonical family: `money-flow-index`
- Category: `indicator`
