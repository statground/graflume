...
![Current Aroon indicator output](../assets/charts/aroon.svg)

This page documents the currently implemented **Aroon indicator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **aroon indicator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
aroon('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · aroon',
  },
  accessibility: {
    label: 'Aroon indicator example',
    description: 'A compiled aroon indicator example using the aroon family.',
  },
});
```
...
- User-facing family: `aroon`
- Quick API: `aroon()`
- Portable mark: `indicator`
- Canonical family: `aroon`
- Category: `indicator`
