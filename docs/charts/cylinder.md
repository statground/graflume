...
![Current Cylinder chart output](../assets/charts/cylinder.svg)

This page documents the currently implemented **Cylinder chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **cylinder chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
cylinder('#chart', data, {
  ...
  title: {
    text: 'Cylinder chart',
    subtitle: 'cartesian · cylinder',
  },
  accessibility: {
    label: 'Cylinder chart example',
    description: 'A compiled cylinder chart example using the cylinder family.',
  },
});
```
...
- User-facing family: `cylinder`
- Quick API: `cylinder()`
- Portable mark: `cylinder`
- Canonical family: `cylinder`
- Category: `cartesian`
...
Builds a column body with sampled top and bottom ellipses to preserve the cylindrical reading in portable primitives. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
