...
Renko and Point & Figure output is path-dependent: each block or column depends on the complete preceding price sequence. Keep generic playback filtering off by default. `frame` and `window` modes distort that path; even a cumulative prefix needs specialist validation of ordering, reversal, brick-size, and warm-up semantics. Inspection/fullscreen/export are view-only and remain safe; see [Common chart interactions](./interactions.md).
