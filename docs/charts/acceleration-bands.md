...
![Current Acceleration bands output](../assets/charts/acceleration-bands.svg)

This page documents the currently implemented **Acceleration bands** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **acceleration bands** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
accelerationBands('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · acceleration-bands',
  },
  accessibility: {
    label: 'Acceleration bands example',
    description: 'A compiled acceleration bands example using the acceleration-bands family.',
  },
});
```
...
- User-facing family: `acceleration-bands`
- Quick API: `accelerationBands()`
- Portable mark: `indicator`
- Canonical family: `acceleration-bands`
- Category: `indicator`
