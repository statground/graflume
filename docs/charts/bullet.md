...
![Current Bullet chart output](../assets/charts/bullet.svg)

This page documents the currently implemented **Bullet chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **bullet chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
bullet('#chart', data, {
  ...
  title: {
    text: 'Bullet chart',
    subtitle: 'cartesian · bullet',
  },
  accessibility: {
    label: 'Bullet chart example',
    description: 'A compiled bullet chart example using the bullet family.',
  },
});
```
...
- User-facing family: `bullet`
- Quick API: `bullet()`
- Portable mark: `bullet`
- Canonical family: `bullet`
- Category: `cartesian`
...
Layers qualitative ranges, the observed bar, and a target rule on the same quantitative scale. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
