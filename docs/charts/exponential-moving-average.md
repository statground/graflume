...
![Current Exponential moving average output](../assets/charts/exponential-moving-average.svg)

This page documents the currently implemented **Exponential moving average** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **exponential moving average** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
exponentialMovingAverage('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · exponential-moving-average',
  },
  accessibility: {
    label: 'Exponential moving average example',
    description:
      'A compiled exponential moving average example using the exponential-moving-average family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Exponential moving average example",
    "description": "A compiled exponential moving average example using the exponential-moving-average family."
  }
}
```
...
- User-facing family: `exponential-moving-average`
- Quick API: `exponentialMovingAverage()`
- Portable mark: `indicator`
- Canonical family: `exponential-moving-average`
- Category: `indicator`
