...
![Current Aroon oscillator output](../assets/charts/aroon-oscillator.svg)

This page documents the currently implemented **Aroon oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **aroon oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
aroonOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · aroon-oscillator',
  },
  accessibility: {
    label: 'Aroon oscillator example',
    description: 'A compiled aroon oscillator example using the aroon-oscillator family.',
  },
});
```
...
- User-facing family: `aroon-oscillator`
- Quick API: `aroonOscillator()`
- Portable mark: `indicator`
- Canonical family: `aroon-oscillator`
- Category: `indicator`
