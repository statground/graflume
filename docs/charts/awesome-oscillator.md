...
![Current Awesome oscillator output](../assets/charts/awesome-oscillator.svg)

This page documents the currently implemented **Awesome oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **awesome oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
awesomeOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · awesome-oscillator',
  },
  accessibility: {
    label: 'Awesome oscillator example',
    description: 'A compiled awesome oscillator example using the awesome-oscillator family.',
  },
});
```
...
- User-facing family: `awesome-oscillator`
- Quick API: `awesomeOscillator()`
- Portable mark: `indicator`
- Canonical family: `awesome-oscillator`
- Category: `indicator`
