...
Waterfall bars are path-dependent: every start/end position depends on prior rows. Do not use generic `frame` or `window` playback filtering. A source-ordered `cumulative` reveal can be considered only after host-side semantic review, with fixed ordering and complete subtotal/total policy. The inspection viewport, fullscreen, and PNG export do not alter the running-total calculation; see [Common chart interactions](./interactions.md).
