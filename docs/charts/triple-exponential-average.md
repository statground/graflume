...
![Current Triple exponential moving average output](../assets/charts/triple-exponential-average.svg)

This page documents the currently implemented **Triple exponential moving average** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **triple exponential moving average** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
tripleExponentialMovingAverage('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · triple-exponential-average',
  },
  accessibility: {
    label: 'Triple exponential moving average example',
    description:
      'A compiled triple exponential moving average example using the triple-exponential-average family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Triple exponential moving average example",
    "description": "A compiled triple exponential moving average example using the triple-exponential-average family."
  }
}
```
...
- User-facing family: `triple-exponential-average`
- Quick API: `tripleExponentialMovingAverage()`
- Portable mark: `indicator`
- Canonical family: `triple-exponential-average`
- Category: `indicator`
