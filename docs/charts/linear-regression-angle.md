...
![Current Linear regression angle output](../assets/charts/linear-regression-angle.svg)

This page documents the currently implemented **Linear regression angle** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **linear regression angle** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
linearRegressionAngle('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · linear-regression-angle',
  },
  accessibility: {
    label: 'Linear regression angle example',
    description:
      'A compiled linear regression angle example using the linear-regression-angle family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Linear regression angle example",
    "description": "A compiled linear regression angle example using the linear-regression-angle family."
  }
}
```
...
- User-facing family: `linear-regression-angle`
- Quick API: `linearRegressionAngle()`
- Portable mark: `indicator`
- Canonical family: `linear-regression-angle`
- Category: `indicator`
