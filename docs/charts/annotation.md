...
Collision avoidance for labels generated by the canonical annotation mark, a side detail panel, range navigation, direct-manipulation editing, and multi-series annotation arbitration are not implemented yet. This canonical mark is an ordered event-series presentation; the separate [top-level callout contract](./interactions.md#legends-highlights-selection-and-callouts) can annotate any family and provides collision-aware perimeter placement, visibility controls, safe multilingual wrapping, and programmatic lifecycle methods.
