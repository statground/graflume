...
![Current Klinger oscillator output](../assets/charts/klinger-oscillator.svg)

This page documents the currently implemented **Klinger oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **klinger oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
klingerOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · klinger-oscillator',
  },
  accessibility: {
    label: 'Klinger oscillator example',
    description: 'A compiled klinger oscillator example using the klinger-oscillator family.',
  },
});
```
...
- User-facing family: `klinger-oscillator`
- Quick API: `klingerOscillator()`
- Portable mark: `indicator`
- Canonical family: `klinger-oscillator`
- Category: `indicator`
