...
![Current Event flags output](../assets/charts/event-flags.svg)

This page documents the currently implemented **Event flags** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this financial chart when the visual relationship represented by **event flags** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
eventFlags('#chart', data, {
  ...
  title: {
    text: 'Event flags',
    subtitle: 'financial · event-flags',
  },
  accessibility: {
    label: 'Event flags example',
    description: 'A compiled event flags example using the event-flags family.',
  },
});
```
...
- User-facing family: `event-flags`
- Quick API: `eventFlags()`
- Portable mark: `flags`
- Canonical family: `event-flags`
- Category: `financial`
...
Anchors compact labeled flags to quantitative or temporal points. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
