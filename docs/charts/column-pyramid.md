...
![Current Column pyramid chart output](../assets/charts/column-pyramid.svg)

This page documents the currently implemented **Column pyramid chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **column pyramid chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
columnPyramid('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'cartesian · column-pyramid',
  },
  accessibility: {
    label: 'Column pyramid chart example',
    description: 'A compiled column pyramid chart example using the column-pyramid family.',
  },
});
```
...
- User-facing family: `column-pyramid`
- Quick API: `columnPyramid()`
- Portable mark: `pyramid`
- Canonical family: `column-pyramid`
- Category: `cartesian`
