...
![Current Flow map output](../assets/charts/flow-map.svg)

This page documents the currently implemented **Flow map** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this map chart when the visual relationship represented by **flow map** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
flowMap('#chart', data, {
  ...
  title: {
    text: 'Flow map',
    subtitle: 'map · flow-map',
  },
  accessibility: {
    label: 'Flow map example',
    description: 'A compiled flow map example using the flow-map family.',
  },
});
```
...
- User-facing family: `flow-map`
- Quick API: `flowMap()`
- Portable mark: `geo-flow`
- Canonical family: `flow-map`
- Category: `map`
...
Projects route endpoints, samples curved paths, and adds directional arrowheads. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
