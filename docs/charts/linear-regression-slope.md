...
![Current Linear regression slope output](../assets/charts/linear-regression-slope.svg)

This page documents the currently implemented **Linear regression slope** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **linear regression slope** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
linearRegressionSlope('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · linear-regression-slope',
  },
  accessibility: {
    label: 'Linear regression slope example',
    description:
      'A compiled linear regression slope example using the linear-regression-slope family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Linear regression slope example",
    "description": "A compiled linear regression slope example using the linear-regression-slope family."
  }
}
```
...
- User-facing family: `linear-regression-slope`
- Quick API: `linearRegressionSlope()`
- Portable mark: `indicator`
- Canonical family: `linear-regression-slope`
- Category: `indicator`
