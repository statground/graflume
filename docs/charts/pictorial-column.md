...
![Current Pictorial column chart output](../assets/charts/pictorial-column.svg)

This page documents the currently implemented **Pictorial column chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this cartesian chart when the visual relationship represented by **pictorial column chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
pictorialColumn('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'cartesian · pictorial-bar',
  },
  accessibility: {
    label: 'Pictorial column chart example',
    description: 'A compiled pictorial column chart example using the pictorial-bar family.',
  },
});
```
...
- User-facing family: `pictorial-column`
- Quick API: `pictorialColumn()`
- Portable mark: `pictorial-bar`
- Canonical family: `pictorial-bar`
- Category: `cartesian`
...
Reuses the canonical repeated-symbol bar compiler. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
