...
![Current Tiled map output](../assets/charts/tiled-map.svg)

This page documents the currently implemented **Tiled map** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this map chart when the visual relationship represented by **tiled map** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
tiledMap('#chart', data, {
  ...
  title: {
    text: 'Tiled map',
    subtitle: 'map · tiled-map',
  },
  accessibility: {
    label: 'Tiled map example',
    description: 'A compiled tiled map example using the tiled-map family.',
  },
});
```
...
- User-facing family: `tiled-map`
- Quick API: `tiledMap()`
- Portable mark: `tiled-map`
- Canonical family: `tiled-map`
- Category: `map`
...
Draws a deterministic portable tile surface and projects point rows on top. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
...
Portable specs do not fetch arbitrary external tiles. The built-in compiler renders a deterministic tile surface; authenticated or remote tile providers belong in an explicit map plugin.
