...
![Current Absolute price oscillator output](../assets/charts/absolute-price-oscillator.svg)

This page documents the currently implemented **Absolute price oscillator** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this indicator chart when the visual relationship represented by **absolute price oscillator** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
absolutePriceOscillator('#chart', data, {
  ...
  title: {
    ...
    subtitle: 'indicator · absolute-price-oscillator',
  },
  accessibility: {
    label: 'Absolute price oscillator example',
    description:
      'A compiled absolute price oscillator example using the absolute-price-oscillator family.',
  },
});
```
...
```json
{
  ...
  "accessibility": {
    "label": "Absolute price oscillator example",
    "description": "A compiled absolute price oscillator example using the absolute-price-oscillator family."
  }
}
```
...
- User-facing family: `absolute-price-oscillator`
- Quick API: `absolutePriceOscillator()`
- Portable mark: `indicator`
- Canonical family: `absolute-price-oscillator`
- Category: `indicator`
