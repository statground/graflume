...
![Current Pareto chart output](../assets/charts/pareto.svg)

This page documents the currently implemented **Pareto chart** family in Graflume `0.1.0-alpha.0`. The image above is generated from the same compiled Scene used by the Canvas renderer.

## When to use it

Use this distribution chart when the visual relationship represented by **pareto chart** is more informative than a plain line, bar, or table. Prefer a simpler chart when the extra geometry does not add analytical meaning.
...
pareto('#chart', data, {
  ...
  title: {
    text: 'Pareto chart',
    subtitle: 'distribution · pareto',
  },
  accessibility: {
    label: 'Pareto chart example',
    description: 'A compiled pareto chart example using the pareto family.',
  },
});
```
...
- User-facing family: `pareto`
- Quick API: `pareto()`
- Portable mark: `pareto`
- Canonical family: `pareto`
...
Combines descending-value columns with a cumulative percentage path in one compiled Scene. The output uses only groups, paths, lines, rectangles, circles, and text, so Canvas, snapshots, export adapters, and future renderers share the same geometry contract.
