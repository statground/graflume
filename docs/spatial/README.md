...
Spatial control visibility is portable and menu-specific. `interaction.controls: false` removes the toolbar, while `true` preserves the complete standard toolbar. An object accepts `orbit`, `pan`, `zoom`, `zoomIn`, `zoomOut`, `reset`, `projection`, `fullscreen`, `export`, and `annotations`. Existing object specs remain compatible: standard menu items default to visible, `annotations` remains opt-in, `zoom` provides the default for both zoom buttons, and `zoomIn` or `zoomOut` can override its own item. Set unwanted standard items to `false`; for example `{ orbit: false, pan: false, zoom: false, reset: false, projection: false, fullscreen: true, export: false }` leaves only fullscreen.

Set `interaction.controls: { annotations: true }` to add the same accessible show/hide button used by Canvas charts. The button is omitted while no annotation exists. If every standard item is hidden and no requested annotation item can be rendered, Graflume omits the entire toolbar rather than drawing an empty control surface. Hiding annotations removes bubbles and connectors only, so highlights, selection, and the authored accessibility description remain. Legend, selection, runtime annotation, and annotation visibility changes are transient and never mutate `getSpec()`; `setSpec()` restores visibility and emits `spec` lifecycle reasons.
