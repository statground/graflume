...
`SpatialSpec.theme` accepts the same `graflume-light`, `graflume-dark`, `ggplot`, or custom theme override as `ChartSpec`. The `ggplot` profile applies its grey panel, white outer surface, typography, HCL categorical colours, continuous ramp, legend, callout, tooltip, and control chrome to every spatial mode. Spatial geometry, camera, lighting, and depth remain spatial because ggplot2 core has no corresponding 3D geometry. Explicit spatial `background`, mark colours, and datum colours take precedence. See [Chart themes](../charts/themes.md).
