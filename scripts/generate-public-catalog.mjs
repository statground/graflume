...
const families = allFamilyIds.map((id, order) => {
  ...
  assert.ok(group && feature, `missing catalog contract for ${id}`);
...
function transformSemantics(entry) {
  ...
  if (id === 'renko' || id === 'point-and-figure') return [`finance:${id}`];
...
assert.equal(manifest.runtimeCapabilities.technicalIndicators.calculations.computed, 17);
assert.equal(manifest.runtimeCapabilities.technicalIndicators.calculations.precomputedRequired, 28);
...
const roadmapSource =
  ...
  features.families
    .map(
      (item) =>
        `| [\`${item.id}\`](../${item.id === 'surface' || item.id === 'volume' || item.id === 'spatial-vector' ? `spatial/${item.id}.md` : `charts/${item.id}.md`}) | ${item.status} | ${list(item.supported)} | ${list(item.p0)} | ${list(item.p1)} | ${list(item.p2)} | ${list(item.dependencies)} |`,
    )
    .join('\n') +
  `\n\n## Candidate contracts\n\n` +
